...
let validate_seg seg len =
  ...
  (* TODO: actually print the segments *)
  if start_pos < 0 then (
    prerr_endline ("Invalid_segmentation (Out_of_bounds seg)");
    exit 0
  ) else if start_pos >= len then (
    prerr_endline ("Invalid_segmentation (Out_of_bounds seg)");
    exit 0
  ) else if end_pos <= start_pos then (
    prerr_endline ("Invalid_segmentation (Non_positive_length seg)");
    exit 0
...
let validate_seg_pair seg1 seg2 len =
  ...
  if end_pos1 >= start_pos2 then
    prerr_endline ("Invalid_segmentation (Bad_separation (seg1, seg2))")
  else ()
...
(*     | () -> true *)
(*     | exception (Invalid_segmentation _) -> false in *)

(*   let validate_raises segs len result = *)
(*     match validate segs len with *)
(*     | () -> false *)
(*     | exception (Invalid_segmentation e) -> e = result in *)

(*   begin *)
(*     assert (validate_ok [] 10); *)
(*     assert (validate_raises [ mk (-1, 1) ] 5 (Out_of_bounds (mk (-1, 1)))); *)
(*     assert (validate_raises [ mk (0, 1) ] 0 (Out_of_bounds (mk (0, 1)))); *)
(*     assert (validate_ok [ mk (0, 1) ] 1); *)
(*     assert (validate_raises [ mk (0, 0) ] 1 (Non_positive_length (mk (0, 0)))); *)
(*     assert (validate_raises [ mk (1, 0) ] 2 (Non_positive_length (mk (1, 0)))); *)
(*     assert (validate_raises [ mk (0, 2); mk (1, 3) ] 4 (Bad_separation (mk (0, 2), mk (1, 3)))); *)
(*     assert (validate_raises [ mk (1, 3); mk (0, 2) ] 4 (Bad_separation (mk (0, 2), mk (1, 3)))); *)
(*     assert (validate_raises [ mk (0, 2); mk (2, 3) ] 4 (Bad_separation (mk (0, 2), mk (2, 3)))); *)
(*     assert (validate_raises [ mk (0, 2); mk (4, 5); mk (2, 3) ] 6 (Bad_separation (mk (0, 2), mk (2, 3)))); *)
(*     assert (validate_raises [ mk (0, 2); mk (4, 5); mk (3, 4) ] 6 (Bad_separation (mk (3, 4), mk (4, 5)))); *)
