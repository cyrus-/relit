...
let menhir_from_module lexer_path parser_path =
  ...
  ^ Path.name lexer_path  ^ ".read;;")
    ...
    |> fun x -> if not x
       then raise (Failure "while trying to execute phrase"));
  (Obj.magic (Toploop.getvalue "value")
   : (Lexing.lexbuf -> Migrate_parsetree.Ast_404.Parsetree.expression))

let parser_for call_record =
  let menhir_parser =
    menhir_from_module call_record.lexer call_record.parser in
  let parser source = source
                      |> Lexing.from_string
                      |> menhir_parser
